...
const ViewGlobalStat = () => {
    ...
    useEffect(() => {
        const fetchStatAndMeta = async () => {
            try {
                const data = await fetchPageItem('globalstats', `${id}`);
                setStat(data);

            } catch (err) {
                console.error(err);
                setError('Failed to fetch globalstat');
            } finally {
                setLoading(false);
            }
        };
    ...
    const fetchLockedValue = async () => {
        try {
            ...
            const result = await fetchLocked('globalstat', `${id}`);
            if (result == 1) {
                setLocked(true);
            } else {
                setLocked(false);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const res = await fetch(`${import.meta.env.VITE_API_URL}/api/globalstats/${id}`, {
                method: 'PUT',
                credentials: 'include',
            ...
            if (res.ok) {
                ...
                alert('Global Stat updated!');
            } else {
                const errorData = await res.json();
                alert(errorData.error || 'Error updating Global Stat');
            }
        } catch (err) {
            console.error(err);
            setError('Failed to update Global Stat');
        }
    };

    return (
        <div className={`page-container ${theme}`}>
            <TitleComp title={`Global Stat | ${id}`}></TitleComp>
            <div
                className="form-meta-container"
                style={{ display: 'flex', justifyContent: 'space-between', gap: '20px' }}
            >
                <PageMetaBox type="globalstat" id={id!} deletePerm='portal.globalstats.delete'/>
                <div style={{ flex: 3 }}>
                    {error && <div className="error-message">{error}</div>}
                    {locked && (
                        <div className="alert alert-warning page-input">
                            This Global Stat is locked and cannot be edited.
                        </div>
                    )}

                    {loading ? (
                        <Spinner type="Global Stat" />
                    ) : stat ? (
